...
⟦_mod-E_|:_[_]⟧ : ∀ {n t} → Unit-E {ℕ.suc n} t → (σ : Notnull) → ∀ k → k ∣ proj₁ σ →
...
⟦ varn p + e        mod-E σ , σ≠0 |: k [ k∣σ ]⟧ q x ρ = begin⟨ ↔-setoid ⟩
  ...
  k ∣ ⟦ t ⟧e (x ∷ ρ) ≡⟨ cong (k ∣_) (lin-ext₁ e x (q ℤ.* σ ℤ.+ x) ρ) ⟩
  k ∣ ⟦ t ⟧e (q ℤ.* σ ℤ.+ x ∷ ρ) ∎ where open ≋-Reasoning
⟦ c [ prf ]*var0+ e mod-E σ , σ≠0 |: k [ k∣σ ]⟧ q x ρ = begin⟨ ↔-setoid ⟩
  ...
  k ∣ c ℤ.* qσ ℤ.+ (c ℤ.* x ℤ.+ (⟦ t ⟧e (x ∷ ρ)))
    ≡⟨ cong (k ∣_) (sym (ZProp.+-assoc (c ℤ.* qσ) (c ℤ.* x) (⟦ t ⟧e (x ∷ ρ)))) ⟩
  k ∣ (c ℤ.* qσ ℤ.+ c ℤ.* x) ℤ.+ (⟦ t ⟧e (x ∷ ρ))
    ≡⟨ cong₂ (λ m n → k ∣ m ℤ.+ n) (sym (ZProp.*-distribˡ-+ c qσ x))
                                        (lin-ext₁ e x (qσ ℤ.+ x) ρ) ⟩
  k ∣ c ℤ.* (qσ ℤ.+ x) ℤ.+ (⟦ t ⟧e (q ℤ.* σ ℤ.+ x ∷ ρ))
