...
div-E (k *var Fin.zero  [ prf ]+ e) = (-, k) , _ [ Ndiv.∣-refl ]*var0+ e
...
div (p :∧ q) =
  let (k , p')  = div p; (l , q') = div q in
  let (d , prf) = LCM.lcm ℤ.∣ proj₁ k ∣ ℤ.∣ proj₁ l ∣ in
  let (prfk , prfl) = LCM.LCM.commonMultiple prf in
  (ℤ.+ d , lcm≠0 (proj₂ k) (proj₂ l)) , (prfk ∣-Div p') :∧ (prfl ∣-Div q')
div (p :∨ q) =
  let (k , p') = div p; (l , q') = div q in
  let (d , prf) = LCM.lcm ℤ.∣ proj₁ k ∣ ℤ.∣ proj₁ l ∣ in
  let (prfk , prfl) = LCM.LCM.commonMultiple prf in
